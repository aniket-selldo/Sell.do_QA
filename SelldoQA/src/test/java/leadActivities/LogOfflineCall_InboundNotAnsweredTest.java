...
public class LogOfflineCall_InboundNotAnsweredTest extends BaseTest {
	...
	public void logOfflineCall_InboundNotAnsweredTest() throws Exception {
		...
		// extentTest.get().log(Status.INFO, "Selecting Conducted sitevisit from
		// more.........");
		...
		// extentTest.get().log(Status.INFO, "Clicking on Call link under activities
		// section button.......");
		leadProfilePage.clickOnCall_d();

		// extentTest.get().log(Status.INFO, "Fetching the text appeared after adding
		// log offline call....");
		...
		// extentTest.get().log(Status.INFO, "Verifying the text under Call
		// activities....");
		Assert.assertEquals(text.split("at")[0].trim(), "You missed call", "text not matching");

		// extentTest.get().log(Status.INFO, "Fetching Incoming text appeared after
		// adding Inbound call....");
		...
		// extentTest.get().log(Status.INFO, "Verifying that Incoming text appeared
		// after adding Inbound call....");
		Assert.assertEquals(croppedDirectionText.trim(), "Incoming  |  Offline  |  Not Ans", "text not matching");

		// extentTest.get().log(Status.INFO, "Fetching text which shows it is an offline
		// call....");
		...
		// extentTest.get().log(Status.INFO, "Verifying text which shows it is an
		// offline call....");
