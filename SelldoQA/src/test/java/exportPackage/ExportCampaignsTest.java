...
public class ExportCampaignsTest extends BaseTest {

	@Test
	public void exportCampaignsTest() throws Exception {
		...
		extentTest.get().log(Status.INFO, "Clicking on Export Campaigns tab.......");
		exportPage.clickOnExportCampaignsTab();

		extentTest.get().log(Status.INFO, "Selecting Campaign from dropdown.......");
		exportPage.selectCampaign();
		...
		extentTest.get().log(Status.INFO, "Selecting Export type as Campaigns.......");
		exportPage.selectExportType_ExportCampaigns();
		...
		String campaign =exportPage.exportLabel.get(0).getAttribute("innerHTML");
		System.out.println(campaign);

		extentTest.get().log(Status.INFO, "Validating that the latest export is made for Campaigns.......");
		//Assert.assertEquals(campaign, "Campaigns - Export 1", "Not matched");
		Assert.assertTrue(driver.findElements(By.cssSelector("#export_type")).get(0).getText().contains("Campaigns - Export "), "Not matched");//Booking Details - Export 1
