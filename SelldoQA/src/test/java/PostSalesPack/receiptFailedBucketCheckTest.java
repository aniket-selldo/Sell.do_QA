...
public class receiptFailedBucketCheckTest extends SetUp{
	...
	public void FailedBucketCount_VerificationTest() throws Exception {
		...
		test = extent.createTest("FailedBucketCount_VerificationTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Getting count of receipts in failed bucket......");
		WebElement count_b = driver.findElement(By.xpath("//div[text()='Failed']/following::span[1]"));
		String leadCount_b = count_b.getText();
		System.out.println(leadCount_b);

		getExtTest().log(Status.INFO, "Clicking on Successful bucket.......");
		postsalesDashboardPage.clickOnFailedReceiptBucket();

		// ..........Getting Count from the label under List DropDownList for the receipts in Failed stage.....
		
		Thread.sleep(5000);

		getExtTest().log(Status.INFO, "Getting Count from the label under List DropDownList for the receipts in Failed stage...");
		...
		getExtTest().log(Status.INFO,
				"Validating count is not same after clicking on demand letters in Failed bucket......"); 
