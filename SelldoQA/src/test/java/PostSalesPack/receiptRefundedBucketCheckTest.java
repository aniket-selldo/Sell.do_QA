...
public class receiptRefundedBucketCheckTest extends SetUp{
	...
	public void RefundedBucketCount_VerificationTest() throws Exception {
		...
		test = extent.createTest("RefundedBucketCount_VerificationTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Getting count of receipts in Refunded bucket......");
		WebElement count_b = driver.findElement(By.xpath("//div[text()='Refunded']/following::span[1]"));
		String leadCount_b = count_b.getText();
		System.out.println(leadCount_b);

		getExtTest().log(Status.INFO, "Clicking on Refunded bucket.......");
		postsalesDashboardPage.clickOnRefundedReceiptBucket();

		// ..........Getting Count from the label under List DropDownList for the receipts in Refunded stage.....
		
		Thread.sleep(5000);

		getExtTest().log(Status.INFO, "Getting Count from the label under List DropDownList for the receipts in Refunded stage...");
		...
		getExtTest().log(Status.INFO,
				"Validating count is not same after clicking on demand letters in Refunded bucket......"); 
