...
public class confirmedBookingsBucketCheckTest extends SetUp {
	...
	public void confirmedBookingsCount_VerificationTest() throws Exception {
		...
		test = extent.createTest("confirmedBookingsCount_VerificationTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Getting count of confrimed bookings bucket......");
		WebElement count_b = driver.findElement(By.xpath("//div[text()='Confirmed Booking Details']/following::span[1]"));
		...
		getExtTest().log(Status.INFO, "getting Confirmed Bookings Count from the label under List DropDownList...");
		...
		getExtTest().log(Status.INFO,
				"Validating count is not same after clicking on confirmed booking bucket......"); 
