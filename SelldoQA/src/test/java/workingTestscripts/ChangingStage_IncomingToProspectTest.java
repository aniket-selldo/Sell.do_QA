...
public class ChangingStage_IncomingToProspectTest extends SetUp {

	final static Logger logger = Logger.getLogger(ChangingStage_IncomingToProspectTest.class);

	// Description: Changing stage from Incoming to Prospect
	...
	public void appLogin() throws Exception {
		...
		login.login(
				property.getProperty("name") + "+"
						+ property.getProperty("user_email_changingStage_IncomingToProspectTest"),
	...
	public void changingStage_IncomingToProspectTest() throws Exception {
		...
		test = extent.createTest("changingStage_IncomingToProspectTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Opening Lead Details page of a lead under Incomimg stage......");
		...
		getExtTest().log(Status.INFO, "Verifying stage changed from Incoming to Prospect.......");
		...
		AssertJUnit.assertEquals(textDropdownObj, "Prospect", "Not matched");
