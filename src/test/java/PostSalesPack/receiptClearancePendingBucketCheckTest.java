...
public class receiptClearancePendingBucketCheckTest extends SetUp{
	...
	public void ClearancePendingBucketCount_VerificationTest() throws Exception {
		...
		test = extent.createTest("ClearancePendingBucketCount_VerificationTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Getting count of receipts in clearance pending bucket......");
		WebElement count_b = driver.findElement(By.xpath("//div[text()='Clearance Pending']/following::span[1]"));
		String leadCount_b = count_b.getText();
		System.out.println(leadCount_b);

		getExtTest().log(Status.INFO, "Clicking on Clearance Pending bucket.......");
		postsalesDashboardPage.clickOnClearancePendingReceiptBucket();

		// ..........Getting Count from the label under List DropDownList for the receipts in Clearance Pending stage.....
		
		Thread.sleep(5000);

		getExtTest().log(Status.INFO, "Getting Count from the label under List DropDownList for the receipts in Clearance Pending stage...");
