...
public class receiptAvailableForRefundBucketCheckTest extends SetUp{
	...
	public void AvailableForRefundBucketCount_VerificationTest() throws Exception {
		...
		test = extent.createTest("AvailableForRefundBucketCount_VerificationTest");
		setExtentTest(test);
		...
		getExtTest().log(Status.INFO, "Getting count of receipts in Available For Refund bucket......");
		WebElement count_b = driver.findElement(By.xpath("//div[text()='Available for Refund']/following::span[1]"));
		String leadCount_b = count_b.getText();
		System.out.println(leadCount_b);

		getExtTest().log(Status.INFO, "Clicking on Successful bucket.......");
		postsalesDashboardPage.clickOnAvailableForRefundReceiptBucket();

		// ..........Getting Count from the label under List DropDownList for the receipts in Available For Refund stage.....
		
		Thread.sleep(5000);

		getExtTest().log(Status.INFO, "Getting Count from the label under List DropDownList for the receipts in Available For Refund stage...");
		...
		getExtTest().log(Status.INFO,
				"Validating count is not same after clicking on demand letters in Available For Refund bucket......"); 
